...
class AccountDeactivationPresenterTest {
    ...
    fun `present - form update`() = runTest {
        ...
        moleculeFlow(RecompositionMode.Immediate) {
            ...
            val initialState = awaitItem()
            assertThat(initialState.deactivateFormState).isEqualTo(DeactivateFormState.Default)
            initialState.eventSink(AccountDeactivationEvent.SetEraseData(true))
            ...
            updatedState.eventSink(AccountDeactivationEvent.SetPassword("password"))
    ...
    fun `present - submit`() = runTest {
        ...
        moleculeFlow(RecompositionMode.Immediate) {
            ...
            val initialState = awaitItem()
            initialState.eventSink(AccountDeactivationEvent.SetPassword("password"))
            skipItems(1)
            initialState.eventSink(AccountDeactivationEvent.DeactivateAccount(isRetry = false))
            val updatedState = awaitItem()
            assertThat(updatedState.accountDeactivationAction).isEqualTo(AsyncAction.ConfirmingNoParams)
            updatedState.eventSink(AccountDeactivationEvent.DeactivateAccount(isRetry = false))
            val updatedState2 = awaitItem()
    ...
    fun `present - submit with error and retry`() = runTest {
        ...
        moleculeFlow(RecompositionMode.Immediate) {
            ...
            // Retry
            finalState.eventSink(AccountDeactivationEvent.DeactivateAccount(isRetry = true))
    ...
    fun `present - submit with error and cancel`() = runTest {
        ...
        moleculeFlow(RecompositionMode.Immediate) {
            ...
            recorder.assertions().isCalledOnce().with(value("password"), value(true))
            // Cancel
            finalState.eventSink(AccountDeactivationEvent.CloseDialogs)
            val finalState2 = awaitItem()
