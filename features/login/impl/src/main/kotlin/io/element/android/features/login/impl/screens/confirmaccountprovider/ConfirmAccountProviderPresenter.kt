...
class ConfirmAccountProviderPresenter(
    ...
    override fun present(): ConfirmAccountProviderState {
        ...
        val homeserverHistory by remember {
            appPreferencesStore.getHomeserverHistoryFlow().map { history ->
                history.map { AccountProvider.Generic(it) }
            }
        ...
        // Everything is rendered without the https:// scheme (added back at connection time).
        val autocompleteCandidates = remember(homeserverHistory) {
            (homeserverHistory + enterpriseService.accountProviderAllowList() + matrixOrgAccountProvider)
        ...
        val accountProviderInput = userInput ?: accountProvider.friendlyServerName()
            ...
            when (event) {
                ...
                is ConfirmAccountProviderEvent.Continue -> {
                    ...
                    changeServerState.eventSink(
                        ChangeServerEvent.ChangeServer(AccountProvider.Generic(host))
                    )
                }
